...
getMessages = (req, res, next) => {
  ...
  db.any('select * from messages').then((data) => {
    res.status(200).json({status: 'success', data: data, message: 'Retrieved all messages'});
  }).catch((err) => {
    return next(err);
  });
};
...
createMessage = (req, res, next) => {
  ...
  db.none('insert into messages(content, score, timestamps)' + 'values(${content}, ${score}, ${timestamps})', req.body).then(() => {
    res.status(200).json({status: 'success', message: 'created one message'});
  }).catch((err) => {
    return next(err);
  });
};
...
vote = (req, res, next) => {
  ...
  db.task(t => {
    return t.one('select * from messages where id = $1', messageId).then(data => {
    ...
    res.status(200).json({status: 'success', message: 'updated message'});
  }).catch((err) => {
    return next(err);
  });
};
